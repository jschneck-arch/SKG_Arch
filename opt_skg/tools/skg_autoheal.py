...
try:
    r = subprocess.run(
        ["/opt/skg/.venv/bin/python", "/opt/skg/tools/assimilation_verify.py"],
        capture_output=True, text=True, timeout=60
    )
    data = json.loads(r.stdout) if r.stdout else {}
    issues = [k for k, v in data.items() if not v.get("ok")]
    if issues:
        print(f"⚠ Assimilation Verify: {len(issues)} broken skills: {issues}")
    else:
        print("✅ All skills verified and callable.")
except Exception as e:
    print(f"assimilation_verify failed: {e}")
