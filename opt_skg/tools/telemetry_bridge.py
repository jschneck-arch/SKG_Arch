...
# === integrate MEP/XRP field physics ===
try:
    from skg import mep, xrp
    rnt_snapshot = safe_read_json("/var/lib/skg/state/rnt_state.json", {})
    mep_state = mep.integrate_rnt_snapshot(rnt_snapshot)
    xrp_state = xrp.integrate_with_mep(mep_state, rnt_snapshot)
    TELEMETRY.update({
        "mep_coupling": mep_state.get("coupling"),
        "xrp_coherence": xrp_state.get("coherence"),
        "field_strength": xrp_state.get("field_strength")
    })
except Exception as e:
    TELEMETRY["field_integration_error"] = str(e)

# === integrate MEP/XRP field physics ===
try:
    from skg import mep, xrp, utils
    rnt_snapshot = utils.safe_read_json("/var/lib/skg/state/rnt_state.json", {})
    mep_state = mep.integrate_rnt_snapshot(rnt_snapshot)
    xrp_state = xrp.integrate_with_mep(mep_state, rnt_snapshot)
    TELEMETRY.update({
        "mep_coupling": round(mep_state.get("coupling", 0.0), 4),
        "xrp_coherence": round(xrp_state.get("coherence", 0.0), 4),
        "field_strength": round(xrp_state.get("field_strength", 0.0), 4)
    })
except Exception as e:
    TELEMETRY["field_integration_error"] = str(e)
